...
def run_remark(args: list[str]) -> str:
    ...
    if result.returncode != 0:
        print(f"remark error: {result.stderr.strip()}", file=sys.stderr)
        sys.exit(1)
...
def fetch(name: str, folder: str, output: str | None = None) -> str:
    ...
    if not rmdoc.exists():
        print(f"Download failed — no {name}.rmdoc found", file=sys.stderr)
        sys.exit(1)

    lines, pdf_path = extract_strokes(rmdoc)
    output_path = output or f"/tmp/{name}-annotated.pdf"
    render_annotations(lines, pdf_path, output_path)
